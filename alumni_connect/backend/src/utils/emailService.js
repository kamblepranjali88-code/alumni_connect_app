    ...
    await sendEmail(alumniEmail, `${studentName} wants to chat (Session ${sessionNumber}) | Alumni Connect`, `
        <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto">
            ...
            <div style="background:#f9fafb;padding:24px;border-radius:0 0 12px 12px;border:1px solid #e5e7eb">
                ...
                <p>Reply to: ${studentEmail}</p>
            </div>
        </div>`, studentEmail);
    console.log(`[Email] Session ${sessionNumber} notification sent to ${alumniEmail}`);
};
    ...
    await sendEmail(toEmail, `New Event: ${eventTitle} | Alumni Connect`, `
        <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto">
            ...
            <div style="background:#f9fafb;padding:24px;border-radius:0 0 12px 12px;border:1px solid #e5e7eb">
                ...
                <h3>${eventTitle}</h3>
                <p>Date: ${dateStr}</p>
                <p>Mode: <span style="background:${modeColor};color:white;padding:2px 10px;border-radius:12px">${eventMode}</span></p>
                <p>Register by: ${deadlineStr}</p>
                <p>${eventDesc}</p>
                <div style="text-align:center">
                    <a href="https://alumni-connect-app-3udd.vercel.app/events.html"
                       style="background:#2563eb;color:white;padding:12px 28px;border-radius:8px;text-decoration:none;font-weight:bold">View and Register</a>
                </div>
            </div>
        </div>`);
};
